...
import type { ComputerStatus } from "@/types/common";
import { formatRelative } from "@/lib/time";
import { useAppState } from "@/store/context";

function statusDot(status: ComputerStatus): "solid" | "hollow" | "pulse" | "muted" {
  if (status === "connected") return "solid";
  if (status === "connecting") return "pulse";
  if (status === "configured") return "hollow";
  return "muted";
}
...
export default function ComputersPage() {
  ...
  const { computers, loading, errors } = state;
  ...
  return (
    <>
      ...
      {loading.computers ? (
        <p style={{ margin: "22px 0 0", fontSize: 12.5, color: "#9a958c" }}>Loading computers…</p>
      ) : errors.computers ? (
        <p style={{ margin: "22px 0 0", fontSize: 12.5, color: "var(--k-danger)" }}>
          Couldn’t load computers: {errors.computers}
        </p>
      ) : (
        <>
          <div
          ...
          >
            {computers.map((computer) => {
              const canStart =
                computer.status === "connected" || computer.status === "configured";
                ...
                >
                  <div style={{ padding: 18 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <Badge dot={statusDot(computer.status)}>
                    ...
                    <div style={{ marginTop: 14, display: "grid", gap: 6 }}>
                      <p style={{ margin: 0, fontSize: 12.5, color: "#6a665f" }}>
                        {[computer.model, computer.os].filter(Boolean).join(" · ") ||
                          "Details will appear when the companion connects."}
                      </p>
                      <p style={{ margin: 0, fontSize: 12.5, color: "#6a665f" }}>
                        Last seen ·{" "}
                        {computer.lastSeenAt ? formatRelative(computer.lastSeenAt) : "Never"}
                      </p>
                    </div>
