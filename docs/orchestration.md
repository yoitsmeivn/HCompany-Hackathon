...
- `hai-desktop` / `holo-desktop` → `LocalDesktopServiceAdapter` subclasses that submit the task to a **local desktop service on `127.0.0.1`** (`poc/hai-desktop` / `poc/holo-desktop`) over the `/tasks` HTTP contract (bearer auth), polling status + safe lifecycle events. Loopback-only; they drive the real machine.
- `nemoclaw-desktop` → the **same HoloDesktop service and `/tasks` contract**, but running inside a remote **NVIDIA NemoClaw sandbox** (isolated virtual desktop) reached over authenticated **HTTPS** instead of the local machine. Reuses `NemoclawDesktopServiceAdapter` (`server/computer/haiDesktopAdapter.ts`); config requires `KYLIAN_DESKTOP_SERVICE_TOKEN` and `https://` for any non-loopback `KYLIAN_NEMOCLAW_DESKTOP_URL`. Sandbox provisioning lives in `nemoclaw/desktop/`.
...
### Live view: desktop screenshots on the landing monitor (opt-in `KYLIAN_LIVE_VIEW`)

Holo is screenshot-driven, so each computer-use step already produces a base64 JPEG observation. With `KYLIAN_LIVE_VIEW=1` (default off) the desktop services (`poc/hai-desktop`, `poc/holo-desktop`) mine that screenshot from the runtime event they already iterate and attach it — newest-only — to the existing `/tasks/:id/events` payload (`TaskEvent.frame`; omitted from the wire when absent via `response_model_exclude_none`). The shared `LocalDesktopServiceAdapter` reads it during its normal poll and emits a **`screen-frame`** runtime event (`shared/runtimeEvents.ts`), which rides the existing per-session SSE (`/api/sessions/:id/events`). `RuntimeEventHub` keeps frames out of the replay buffer and remembers only the latest, so a (re)connecting client renders the current desktop immediately. On the frontend it lands in `store.live[id].frame` and renders as an `<img>` in `LiveFeed`; the landing page (`src/components/marketing/LiveMonitor.tsx`) watches the monitor stream and swaps the marketing preview for this live view while a call is active. When the flag is off, no screenshot is extracted or emitted and every API is byte-for-byte unchanged. The `frame` field is the single seam a future LiveKit/WebRTC video provider would replace.
